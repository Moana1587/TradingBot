...
export class WebUI extends EventEmitter {
  ...
  private async handleTradeRequest(ws: any, tradeData: any): Promise<void> {
    try {
      const { type, protocol, mint } = tradeData;
      ...
      if (type === 'buy') {
        // Get tracked token information to find target wallet's token amount
        const trackedToken = priceMonitor.getTrackedToken(mint);

        if (!trackedToken || !trackedToken.targetWalletTokenAmount) {
          ws.send(
            JSON.stringify({
              messageType: 'tradeResult',
              success: false,
              error: 'Target wallet token amount not found. Token may not be tracked yet.',
            }),
          );
          return;
        }
        ...
        // For sell, get actual token balance from wallet
        ...
        // Sell all tokens
        const sellTokenAmount = actualTokenBalance;

        logger.info('WebUI', `Selling all tokens: ${sellTokenAmount.toString()}`);
      ...
      if (isOwnTrade) {
        logger.info(
          'WebUI',
          `Own trade detected: ${tradeEvent.type} ${tradeEvent.mint.slice(0, 8)}...`,
        );
      ...
      if (result.success) {
        ...
        if (tradeEvent.type === 'buy') {
          ...
          const buyPrice =
            Number(tradeEvent.solAmount) / Number(tradeEvent.tokenAmount) / LAMPORTS_PER_SOL; // Price per token

          // Track buy fees and token amounts
          priceMonitor.updateBuyFees(
            tradeEvent.mint,
            totalBuyAmount,
            totalBuyFee,
            buyTokenAmount,
