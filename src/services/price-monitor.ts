...
export class PriceMonitor extends EventEmitter {
  ...
  private scheduleReconnect(): void {
    ...
    logger.info('PriceMonitor', `Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`);
        ...
        if (mint) {
          ...
          if (token) {
            ...
            this.updatePriceFromAccount(mint, token, accountValue.data).catch((error) => {
              logger.debug(
                'PriceMonitor',
                `Error updating price for ${mint.slice(0, 8)}...`,
                error,
              );
            });
          }
   ...
   */
  private async fetchTokenLaunchTime(
    mint: string,
    protocol: 'pumpfun' | 'pumpamm',
    bondingCurveAddress?: string,
    poolAddress?: string,
    ...
    try {
      ...
      const transaction = await connectionManager.connection.getTransaction(oldestSignature, {
      ...
      logger.debug(
        'PriceMonitor',
        `Error fetching token launch time for ${mint.slice(0, 8)}...`,
        error,
      );
      return undefined;
    }
  ...
  async trackToken(
    ...
    try {
      ...
      const tokenLaunchTime = await this.fetchTokenLaunchTime(
        mint,
        protocol,
        bondingCurveAddress,
        poolAddress,
  ...
  updateSellTime(
    ...
    if (token) {
      ...
      logger.debug(
        'PriceMonitor',
        `Updated sell time for ${mint.slice(0, 8)}...: ${new Date(sellTime).toISOString()}`,
      );
    }
  }
   ...
   */
  private async getPumpFunPriceAndLiquidity(
    accountData?: string | string[],
